...
$ket=$_POST['ket'];
$tgl=date('d n Y');
$sql=mysqli_query($koneksi, "insert into galery(nm_galery,ket,tgl_galery,gambar)values('$nm_galeri','$ket'
,'$tgl','$nama_file')");
...
{
$unlink=mysqli_query($koneksi, "select * from galery where id_galery='$_GET[id]'");
...
if(empty($lokasi_file))
{
mysqli_query($koneksi, "update galery set nm_galery='$nm_galeri',
ket='$ket'
where id_galery='$id'");
}else{
mysqli_query($koneksi, "update galery set nm_galery='$nm_galeri',
...
mysqli_query($koneksi, "delete from galery where id_galery='$_GET[id]'"); header('location:server.php?module='.$module);
}
//Ganti Password
elseif ($module=='user' and $act=='gantipwd') {
	...
	$baru_banget=$baru; if ( $lama == $lama2)
{
if ($baru == $baru2)
{
if($set)
